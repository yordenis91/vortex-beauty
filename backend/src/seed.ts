...
async function seedKnowledgeBaseCategories(adminUserId: string) {
  try {
    const categories = [
      {
...
async function seedSampleArticles(adminUserId: string) {
  try {
    // Obtener la categoría "Getting Started"
    const gettingStartedCategory = await prisma.category.findFirst({
      where: {
        name: 'Getting Started',
        type: 'KNOWLEDGE_BASE',
      },
